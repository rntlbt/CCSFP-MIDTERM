...
function get_total_row($pdoConnect)
{
  ...
  $pdoResult->execute(array(":status" => "pending"));
...
';
  while($row=$statement->fetch(PDO::FETCH_ASSOC))
  {
    ...
    <tr>
      ...
      <td>
      <button type="button" class="btn btn-success V"><a href="controller/payment-controller?id='. $row["id"].'&user_id='. $row["user_id"].'&package_id='. $row["package_id"].'&accept_payment=1" class="accept_payment">Accept</a></button>
      <button type="button" class="btn btn-danger V"><a href="controller/payment-controller?id='. $row["id"].'&user_id='. $row["user_id"].'&package_id='. $row["package_id"].'&decline_payment=1" class="decline_payment">Decline</a></button>
      </td>
