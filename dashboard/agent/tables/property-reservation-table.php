...
function get_total_row($pdoConnect)
{
  ...
  $pdoResult->execute(array(":agent_id" =>$_SESSION['agent_session'], ":status" => "pending"));
...
$statement->execute(array(":agent_id" =>$_SESSION['agent_session'],":status" => "pending"));
...
';
  while($row=$statement->fetch(PDO::FETCH_ASSOC))
  {
    ...
    if($row["status"] == "pending"){
      $status = '<button type="button" class="btn btn-warning N">Pending</button>';
    }
    ...
    <tr>
      ...
      <td>'.$row["message"].'</td>
      <td>
        <button type="button" class="btn btn-primary V"><a href="controller/property-controller?property_reservation_id='.$row["id"].'&send_email=1" class="send">Send Email</a></button>
      </td>        
    </tr>
    ';
