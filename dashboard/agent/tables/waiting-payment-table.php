...
function get_total_row($pdoConnect)
{
  ...
  $pdoResult->execute(array(":agent_id" =>$_SESSION['agent_session'], ":status" => "send_email"));
...
$statement->execute(array(":agent_id" =>$_SESSION['agent_session'],":status" => "send_email"));
...
';
  while($row=$statement->fetch(PDO::FETCH_ASSOC))
  {
    ...
    if($row["status"] == "send_email"){
      $status = '<button type="button" class="btn btn-warning N">Waiting Payment</button>';
    }
    ...
    <tr>
      ...
      <td>'.$row["message"].'</td>
      <td>
        <button type="button" class="btn btn-success V"><a href="controller/property-controller?property_reservation_id='.$row["id"].'&confirm_payment=1" class="confirm_payment">Confirm Payment</a></button>
        <button type="button" class="btn btn-danger V"><a href="controller/property-controller?property_reservation_id='.$row["id"].'&delete_request=1" class="delete_request">Delete</a></button>
      </td>        
    </tr>
    ';
...
<script>
        ...
        $('.delete_request').on('click', function (e) {
            ...
            swal({
                title: "Sure?",
                text: "Do you want to delete this user reservation?",
                icon: "warning",
                buttons: true,
                dangerMode: true,
            })
        ...
        $('.confirm_payment').on('click', function (e) {
            ...
            swal({
                title: "Sure?",
                text: "Do you want to confirm the payment of this reservation?",
                icon: "warning",
                buttons: true,
                dangerMode: true,
            })
